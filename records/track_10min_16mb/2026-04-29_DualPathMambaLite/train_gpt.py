...
class Hyperparameters:
    architecture_name = "dual_path_mamba_lite"
...
class MambaLiteBlock(nn.Module):
    ...
    def __init__(self, dim: int, conv_kernel: int, use_triton: bool):
        ...
        self.conv = nn.Conv1d(dim, dim, conv_kernel, groups=dim, bias=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return x + self.resid_scale.to(dtype=x.dtype)[None, None, :] * self.out_proj(y)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.use_mamba_path = use_mamba_path
        ...
        self.mamba_branch = MambaLiteBranch(
            model_dim,
            mamba_dim,
            mamba_layers,
            mamba_conv_kernel,
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.mamba_branch is not None:
            mamba_out = self.mamba_branch(x0)
        else:
            mamba_out = None
        skips: list[Tensor] = []
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        use_mamba_path=args.use_mamba_path,
    ...
    # - matrix params in transformer/Mamba-lite body use MATRIX_LR via Muon
    # - vectors/scalars use SCALAR_LR via Adam
    body_named_params = list(base_model.blocks.named_parameters())
    if base_model.mamba_branch is not None:
        body_named_params.extend((f"mamba_branch.{name}", param) for name, param in base_model.mamba_branch.named_parameters())
    matrix_params = [
        p
    ...
    scalar_params.append(base_model.fusion_gate)
    ...
    if master_process:
        ...
        total_quant_bytes = quant_file_bytes + code_bytes
        log0(f"Total submission size int8+zlib: {total_quant_bytes} bytes")
        if args.enforce_artifact_budget and total_quant_bytes > ARTIFACT_LIMIT_BYTES:
            raise RuntimeError(
                f"artifact budget exceeded: {total_quant_bytes} > {ARTIFACT_LIMIT_BYTES}; "
                "try lower MAMBA_DIM, MAMBA_LAYERS, or NUM_LAYERS"
            )
