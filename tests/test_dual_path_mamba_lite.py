...
class DualPathMambaLiteTest(unittest.TestCase):
    def test_dual_path_mamba_lite_is_declared_and_enabled_by_default(self) -> None:
        ...
        self.assertIn('mamba_dim = int(os.environ.get("MAMBA_DIM", 64))', text)
        self.assertIn('mamba_layers = int(os.environ.get("MAMBA_LAYERS", 1))', text)
        self.assertIn('mamba_mixer = os.environ.get("MAMBA_MIXER", "triton_conv1d")', text)
    ...
    def test_default_mamba_mixer_uses_triton_kernel_with_conv1d_fallback(self) -> None:
        text = _script_text()
        self.assertIn("@triton.jit", text)
        self.assertIn("class TritonCausalDepthwiseConv1d", text)
        self.assertIn("def causal_depthwise_conv1d", text)
        self.assertIn('elif mixer == "triton_conv1d":', text)
        self.assertIn('if mixer == "conv1d":', text)
    ...
    def test_default_shape_has_room_under_16mb_artifact_cap(self) -> None:
        ...
        branch_payload = (
            ...
            + layers * ((2 * mamba_dim * mamba_dim * 2) + (mamba_dim * mamba_dim * 2) + (mamba_dim * 7 * 2) + (3 * mamba_dim * 2))
            + model_dim * 2
        )
